...
double ref_v = 40 * 0.44704; // mph -> m/s

// indices in vars and constraints
int x_start = 0;
...
vector<double> MPC::Solve(const Eigen::VectorXd &state, const Eigen::VectorXd &coeffs) {
  ...
  trajectory_ys = {};
  for (int t = 0; t < N; t ++) {
    trajectory_xs.push_back( solution.x[x_start + t] );
    trajectory_ys.push_back( solution.x[y_start + t] );
  }
...
Eigen::VectorXd MPC::StateAfterLatency(const Eigen::VectorXd &state, double delta, double a) {
  double x = state[0];
  double y = state[1];
  double psi = state[2];
  double v = state[3];
  ...
  Eigen::VectorXd new_state(state.size());
  new_state << x + v * cos(psi) * latency / 1000,
               y + v * sin(psi) * latency / 1000,
               psi + v / Lf * delta * latency / 1000,
               v + a * latency / 1000,
               cte + v * sin(epsi) * latency / 1000,
               epsi + v * delta / Lf * latency / 1000;
